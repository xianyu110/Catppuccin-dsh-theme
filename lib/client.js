    ...
    "tokens": {
      ...
      "--shiki-token-link": "#f5e0dc"
		...
		function apply(ctx) {
			...
			// dsh only paints user-message bubbles (specific-bubble); assistant
			// replies sit directly on the canvas. Inject surfaces for both
			// roles via stable data attributes / CSS-module suffixes, colored
			// from our own injected theme variables so they adapt per flavor:
			// user messages get a brand tint, assistant replies a neutral
			// surface one step above the background.
			ctx.effect(() => {
