	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const SURFACE_RULES = [
				...
				[
					"[class$=\"_thinkBody\"]",
					"  color: color-mix(in srgb, var(--dsw-ctp-sky) 72%, var(--dsw-alias-label-tertiary));"
				],
				// tool call rows: the Think label in sky, executor
				// labels in peach, so the message flow reads in two accents
				// instead of all-gray
				[
					"[class$=\"_QWLzlG_title\"]",
					"  color: var(--dsw-ctp-sky);"
				...
				// whole new-session button warms on hover so the leading
				// plus icon follows the label (the icon inherits color)
				[
				...
				[
					"[class$=\"ydkMvW_header\"]:hover, [class$=\"ydkMvW_title\"]:hover, [class$=\"ydkMvW_close\"]:hover",
					"  color: var(--dsw-alias-brand-primary);"
				],
