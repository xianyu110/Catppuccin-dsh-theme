		...
		function apply(ctx) {
			...
			const SURFACE_RULES = [
				[
					...
					"  background: color-mix(in srgb, var(--dsw-alias-brand-primary) 20%, var(--dsw-alias-bg-layer-2));"
				],
				...
				[
					"[class$=\"_thinkBody\"]",
					"  color: color-mix(in srgb, var(--dsw-ctp-peach) 72%, var(--dsw-alias-label-tertiary));"
				],
				// tool call rows: the Think label in peach, executor
				// labels in sky (echoing the composer add button), so the
				// message flow reads in two accents instead of all-gray
				[
					"[class$=\"_QWLzlG_title\"]",
				...
				// code block language tag (python/json/...): peach chip
				[
					"[class$=\"_infostring\"]",
					"  color: var(--dsw-ctp-peach);"
				],
				// message timestamp separators: soft lavender
				[
					"[class$=\"_timeStart\"]",
				...
				[
					...
					"  color: color-mix(in srgb, var(--dsw-ctp-sky) 55%, var(--dsw-alias-label-tertiary));"
				],
				[
					"[class$=\"QWLzlG_summary\"]",
					"  color: color-mix(in srgb, var(--dsw-ctp-peach) 55%, var(--dsw-alias-label-tertiary));"
				],
				...
				// word-match: these labels carry a second modifier class
				// (…_wide), so a $= suffix match would miss them entirely.
				// Section label in peach so it no longer collides with the
				// lavender session times.
				[
					"[class~=\"qDHVXG_sectionLabel\"]",
					"  color: var(--dsw-ctp-peach);"
				],
				[
				...
				// homepage: gradient headline so the empty workspace still
				// shows off the palette, workspace label in lavender, the
				// preview badge in warning yellow
				[
					"[class$=\"pXSMma_headlineText\"]",
					[
						"  background: linear-gradient(90deg, var(--dsw-ctp-peach), var(--dsw-ctp-sky), var(--dsw-alias-brand-primary));",
				...
				// composer selectors: model effort in lavender (warning yellow
				// was too loud on the gray card), workspace and model labels
				// warm to mauve on hover (constant color stays neutral —
				// earlier "cyan" complaints were about the trigger itself,
				// not hover)
				[
